...
my $roi_group;
my $corr = 0;
$roi_list = $ENV{'PIPEDIR'}.'/lib/jhu.'.$atlas.'.rois.list';
$roi_group = $atlas.'_comp';
my $roi_dir = $ENV{'PIPEDIR'}.'/lib/jhu/';
my $jhu = "JHU_".$roi_group;
open DIRF, "<$roi_list" or die "No such ROIs definition\n";
my %masks;
while (<DIRF>) {
	my ($roi,$def) = /(.*):(.*)/;
	$masks{$roi} = $def;
...
my %lort = ('jhuwm' => 'tracts', 'icbm' => 'labels');
...
foreach my $rmask (sort keys %masks){
	if($with_sd){
		print OF ";$rmask","_FA_Mean;","$rmask","_FA_STD",";$rmask","_MD_Mean;","$rmask","_MD_STD";
	}else{
		print OF ";$rmask","_FA",";$rmask","_MD";
	}
}
print OF "\n";

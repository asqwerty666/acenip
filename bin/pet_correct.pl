...
my %ptask = ( 'job_name' => 'pet_reg_'.$study,
	'time' => $time,
	'mem_per_cpu' => '4G',
...
foreach my $subject (@pets){
	...
	unless ($movcor){
		# Ahora si $movcor == 0 tomo el combined y hago un registro simple
			...
			if ($alt){
				...
				# si no se logra registrar bien
                        	$order = $ENV{'PIPEDIR'}."/bin/pet_reg_alt.sh ".$study." ".$subject." ".$w_dir." ".$spet{'combined'}." ".$smri{'T1w'}[0]." 0";
			}else{
				$order = $ENV{'PIPEDIR'}."/bin/pet_reg.sh ".$study." ".$subject." ".$w_dir." ".$spet{'combined'}." ".$smri{'T1w'}[0]." 0";
			}
			$ptask{'command'} = $order;
		...
		if($spet{'single'} && $smri{'T1w'}){
			$ptask{'command'} = $ENV{'PIPEDIR'}."/bin/pet_reg.sh ".$study." ".$subject." ".$w_dir." ".$spet{'single'}." ".$smri{'T1w'}[0]." 1";
			$ptask{'filename'} = $outdir.'/'.$subject.'_pet_reg.sh';
			$ptask{'cpus'} = 4;
			$ptask{'output'} = $outdir.'/pet_reg_'.$subject;
			send2slurm(\%ptask);
...
my %final = ( 'command' => $ENV{'PIPEDIR'}."/bin/make_pet_report.pl ".$study,
	'filename' => $outdir.'/pet_report.sh',
	'job_name' => 'pet_reg_'.$study,
	'time' => '2:0:0',
	'mailtype' => 'FAIL,END',
	'output' => $outdir.'/pet_report',

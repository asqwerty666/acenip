...
#		${ANTS_PATH}/antsRegistrationSyNQuick.sh -d 3 -f ${wdir}/${id}_struc_croped.nii.gz -m ${x%.nii.gz}_mod.nii.gz -t a -o ${x%.nii.gz}_movingToCroped_;
		${ANTS_PATH}/antsRegistration --verbose 1 --dimensionality 3 --float 0 --collapse-output-transforms 1 --output [ ${x%.nii.gz}_movingToCroped_,${x%.nii.gz}_movingToCroped_Warped.nii.gz,${x%.nii.gz}_movingToCroped_InverseWarped.nii.gz ] --interpolation Linear --use-histogram-matching 0 --winsorize-image-intensities [ 0.005,0.995 ] --initial-moving-transform [ ${td}/${id}_struc_croped.nii.gz,${x},1 ] --transform Rigid[ 0.1 ] --metric MI[ ${td}/${id}_struc_croped.nii.gz,${x},1,32,Regular,0.5 ] --convergence [ 1000x500x250x0,1e-6,10 ] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox --transform Affine[ 0.1 ] --metric CC[ ${td}/${id}_struc_croped.nii.gz,${x},0.5,4 ] --convergence [ 1000x500x250x0,1e-6,10 ] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox
		...
		${ANTS_PATH}/antsRegistration --verbose 1 --dimensionality 3 --float 0 --collapse-output-transforms 1 --output [ ${x%.nii.gz}_movingToFixed_,${x%.nii.gz}_movingToFixed_Warped.nii.gz,${x%.nii.gz}_movingToFixed_InverseWarped.nii.gz ] --interpolation Linear --use-histogram-matching 0 --winsorize-image-intensities [ 0.005,0.995 ] --initial-moving-transform [ ${wdir}/${id}_struc.nii.gz,${x},1 ] --transform Rigid[ 0.1 ] --metric MI[ ${wdir}/${id}_struc.nii.gz,${x},1,32,Regular,0.5 ] --convergence [ 1000x500x250x0,1e-6,10 ] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox --transform Affine[ 0.1 ] --metric CC[ ${wdir}/${id}_struc.nii.gz,${x},0.5,4 ] --convergence [ 1000x500x250x0,1e-6,10 ] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox

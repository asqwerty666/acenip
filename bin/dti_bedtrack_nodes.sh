...
Usage() { 
    echo "" 
    echo "Usage: dti_bedtrack.sh  <project> <name> <working dir> <network dir>"
    echo "" 
...
for x in `find ${net_dir} -name "*.nii"`; do
	...
	antsApplyTransforms -d 3 -i ${td}/${node}_warped.nii.gz -r ${td}/hifi_b0.nii.gz -t ${td}/${pollo}_epi_reg_ANTS.mat -o ${td}/${pollo}_${node}.nii.gz
...
${FSLDIR}/bin/probtrackx2 --opd --forcedir -s ${bd}.bedpostX/merged -m ${w_dir}/${pollo}_dti_brain_mask -x ${td}/${pollo}_mask.list --dir=${td}/probtrack_out

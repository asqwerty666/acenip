...
while (@ARGV and $ARGV[0] =~ /^-/) {
    $_ = shift;
    last if /^--$/;
    if (/^-h/) { print_help $ENV{'PIPEDIR'}.'/doc/make_dti_report.hlp'; exit;}
}
my $study = shift;
unless ($study) { print_help $ENV{'PIPEDIR'}.'/doc/make_dti_report.hlp'; exit;}
...
my @names = find(file => 'name' => "*mni_to_b0.nii.gz", in => $w_dir);
foreach my $name (sort @names){
	(my $warped = $name) =~ s/mni_to_b0/hifi_b0/; 
	$order .= $warped.' '.$name." ";
}
chdir $w_dir;
system($order);
